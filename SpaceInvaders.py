def second_level():
    global enemy_strela
    first_level()
    enemy_strela = game.create_sprite(nepritel1.get(LedSpriteProperty.X),
        nepritel1.get(LedSpriteProperty.Y))
    enemy_strela.change(LedSpriteProperty.BRIGHTNESS, 80)
    if lod.is_touching_edge():
        for index in range(4):
            enemy_strela.change(LedSpriteProperty.Y, 1)
            basic.pause(200)
        enemy_strela.delete()
